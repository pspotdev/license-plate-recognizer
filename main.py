...
def extract_lpd(image_data: bytes, filename: str, model, session_crnn, crnn_input_name) -> Dict:
    try:
...
# Initialize ThreadPoolExecutor for concurrent image processing
executor = ThreadPoolExecutor(max_workers=4)

def process_single_image(image_data: bytes, filename: str) -> Dict:
    """
    Process a single image using the existing extract_lpd function.
    Synchronous function to run in ThreadPoolExecutor.
    """
    try:
        result = extract_lpd(image_data, filename, model, session_crnn, crnn_input_name)
...
async def process_images(files: List[UploadFile] = File(...)):
    """
    Process multiple uploaded images for license plate detection and recognition.
    Returns detected plates with text and confidence.
    """
    # Validate file types
    for file in files:
    ...
    try:
        # Read all image bytes asynchronously
        image_data_list = [(await file.read(), file.filename) for file in files]
        
        # Process images concurrently using ThreadPoolExecutor
        loop = asyncio.get_event_loop()
        tasks = [
            loop.run_in_executor(executor, process_single_image, image_data, filename)
...
async def process_image(file: UploadFile = File(...)):
    """
    Process a single uploaded image for license plate detection and recognition.
